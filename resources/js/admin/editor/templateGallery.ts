...
export const GALLERY: GalleryTemplate[] = [
    ...
    },
    {
        ...
        build: () => [
            ...
            kpi('mainwp', 'vulnerabilities_count', 'Vulnerabilidades', { x: 3, y: 9, w: 3, h: 4 }),
            ...
            spec({
                ...
                props: { title: 'Estado de seguridad' },
                layout: { x: 0, y: 13, w: 6, h: 7 },
            }),
            spec({
                type: 'worklog_timeline',
    ...
    },
    {
        ...
        description: 'Demuestra la vigilancia continua: el registro de escaneos de seguridad de Wordfence del periodo, con fecha y resultado de cada análisis.',
        build: () => [
            header('Análisis de seguridad'),
            spec({
                type: 'kpi',
                binding: { source: 'mainwp', metric: 'wordfence_scans_count' },
                props: { label: 'Escaneos realizados' },
                style: { format: 'number' },
                layout: { x: 0, y: 2, w: 4, h: 4 },
            }),
            spec({ type: 'security_shield', props: { title: 'Tu sitio, analizado' }, layout: { x: 4, y: 2, w: 8, h: 4 } }),
            summary({ x: 0, y: 6, w: 12, h: 4 }),
            spec({
                type: 'table',
                binding: { source: 'mainwp', metric: 'wordfence_scans' },
                props: { title: 'Escaneos de Wordfence' },
                layout: { x: 0, y: 10, w: 12, h: 8 },
            }),
            cta(18),
        ],
    ...
    },
    {
        ...
        description: 'Estado real de los respaldos del sitio (último respaldo, antigüedad, tamaño) más la salud del WordPress: versiones, plugins, actualizaciones y almacenamiento. Vía el plugin Agente Imagina.',
        build: () => [
            ...
            spec({
                type: 'table',
                binding: { source: 'site_agent', metric: 'backup_status' },
                props: { title: 'Estado de respaldos' },
                layout: { x: 0, y: 10, w: 6, h: 8 },
            }),
            spec({
                type: 'table',
                binding: { source: 'site_agent', metric: 'recent_backups' },
                props: { title: 'Respaldos recientes' },
                layout: { x: 6, y: 10, w: 6, h: 8 },
            }),
            spec({
                type: 'table',
                binding: { source: 'site_agent', metric: 'site_health' },
                props: { title: 'Salud del sitio' },
    ...
    },
    {
        ...
        description: 'Panel 360 del sitio vía el Agente Imagina: spam bloqueado, solicitudes recibidas, auditoría de seguridad, estado de rendimiento, limpieza de base de datos y operativa de tienda. Los bloques sin datos se ocultan solos.',
        build: () => [
            ...
            spec({
                type: 'table',
                binding: { source: 'site_agent', metric: 'security_audit' },
                props: { title: 'Auditoría de seguridad' },
                layout: { x: 0, y: 10, w: 6, h: 8 },
            }),
            spec({
                type: 'table',
                binding: { source: 'site_agent', metric: 'performance_status' },
                props: { title: 'Estado de rendimiento' },
                layout: { x: 6, y: 10, w: 6, h: 8 },
            }),
            spec({
                type: 'table',
                binding: { source: 'site_agent', metric: 'db_cleanup' },
                props: { title: 'Limpieza de base de datos' },
                layout: { x: 0, y: 18, w: 6, h: 8 },
            }),
