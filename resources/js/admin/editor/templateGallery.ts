...
export const GALLERY: GalleryTemplate[] = [
    ...
    {
        key: 'backups',
        name: 'Respaldos y mantenimiento (MainWP)',
        description: 'La red de seguridad: copias de seguridad creadas y tareas de mantenimiento ejecutadas este mes, más el registro de lo realizado.',
        build: () => [
            header('Respaldos y mantenimiento'),
            spec({
                type: 'kpi',
                binding: { source: 'mainwp', metric: 'backups_count' },
                props: { label: 'Respaldos creados' },
                style: { format: 'number' },
                layout: { x: 0, y: 2, w: 4, h: 4 },
            }),
            spec({
                type: 'kpi',
                binding: { source: 'mainwp', metric: 'maintenance_count' },
                props: { label: 'Tareas de mantenimiento' },
                style: { format: 'number' },
                layout: { x: 4, y: 2, w: 4, h: 4 },
            }),
            spec({
                type: 'kpi',
                binding: { source: 'mainwp', metric: 'updates_applied' },
                props: { label: 'Actualizaciones aplicadas' },
                style: { format: 'number' },
                layout: { x: 8, y: 2, w: 4, h: 4 },
            }),
